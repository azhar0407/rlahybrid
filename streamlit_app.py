...
try:
    with st.spinner('Menarik data dari Supabase & Menghitung AI...'):
        ...
        if not df_db.empty:
            ...
            # 4. Wilder's ATR 14 (Perbaikan Krusial!)
            ...
            # Fungsi Wilder's Smoothing Khusus (AmiBroker Style)
            ...
            if df_today['MA200'].isnull().all():
                ...
                # Syarat Sesuai AFL (Termasuk yang menyebabkan BJBR gagal di Amibroker)
                ...
                cond_liq = df_today['AvgVal20'] > liq_min
                cond_rs50 = df_today['RS50'] > 0
                cond_late = df_today['ROC20'] < 25  # Batas Telat ROC20
                
                final_signal = df_today[cond_trend & cond_vol & cond_foreign & cond_vbf & cond_liq & cond_rs50 & cond_late]
                ...
                if not final_signal.empty:
                    ...
                    cols_to_show = ['Ticker', 'Close', 'NetForeign', 'Vol_Surge', 'RS50']
    ...
    st.info("Server siap. Silakan login sebagai Admin.")
